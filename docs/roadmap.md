...
4. **В работе.** Создан отдельный сохраняемый диск ext4 для `/root`, `/home`
   и `/var/lib`. Далее: выбрать libc/init, добавить системные утилиты.
...
Для этапа 4: выбрать подход к libc и сервисам с учётом текущей статической
glibc/BusyBox, описать постоянные базовые настройки и порядок добавления
утилит. Продолжать проверять загрузку после каждого изменения. Постоянный
системный rootfs и установщик относятся к более поздним этапам.
