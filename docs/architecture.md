...
и `/var/lib`. Если диск запрошен, но недоступен, загрузка останавливается
вместо запуска без сохранения данных. Затем управление получает BusyBox init.
`inittab` запускает shell на ttyS0 и восстанавливает его после выхода. Init
также обрабатывает завершение процессов и штатное выключение.
